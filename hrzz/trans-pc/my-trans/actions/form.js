...
export default class FormAction {
    ...
    confirmAdd = async (param) => {
        ...
        const {transMode, transType} = param;

        let res = await dispatch({
            type: 'main/addNew',
            payload: {transMode, transType: transType.refpk}
        });
        if (res.success) {
        ...
        if (res.success) {
            ...
            await dispatch({
                ...
                payload: {
                    page: 'detail',
                    isEdit: true,
                    transMode: res.data.formData.card.rows[0].values.stapply_mode.value
                }
            });
            action.btnAct.updateBtnStatus();
            // 是否禁用编码
    ...
    confirmSave = async () => {
        ...
        if (res.success) {
            ...
            form.setAllFormValue({card: res.data.formData.card});
            const billId = res.data.formData.card.rows[0].values.pk_hi_stapply.value;
    ...
    checkOneBill = async (record) => {
        const {props, action} = this.comp;
        const {dispatch, cardPagination, meta, form} = props;
        let res = await dispatch({
        ...
        if (res.success) {
            ...
            cardPagination.setCardPaginationId({
                id: res.data.formData.card.rows[0].values.pk_hi_stapply.value,
                status: 1
            });
            form.setFormStatus('card', 'browse');
            form.openArea('psninfo');
