                ...
                'rightItems': [
                    {
                        ...
                        'iconColor': '#ffff00',
                        'callback': JSON.stringify(function () {
                            console.log("测试友空间头部按钮1")
                        })
                    },
                    {
                        'title': 'hello',
...
const openWindow = (paramobj, callbackFun) => {
    connectWebViewJavascriptBridge(function (YonYouJSBridge) {
        ...
        if (callbackFun) {
            YonYouJSBridge.registerHandler('resume', () => {
                callbackFun()
            });
        }
        if (typeof paramobj == 'string') {//字符串，表示url地址
            ...
            var data = {
                ...
                'parameters': {
                    ...
                    'navShow': paramobj.navShow || '2',//默认不显示原生头部，防止出现链接头部
                    'fullScreen': '0',
                    'url': paramobj.url,
                    'onresume': paramobj.onresume
                }
            }
            //非必填字段
...
// 调用方法  NativeObj.backbutton("",function(){ alert("android的返回按钮")})
const backbutton = (paramobj, callbackFun) => {
    connectWebViewJavascriptBridge(function (YonYouJSBridge) {
        try {
            YonYouJSBridge.init(function (message, responseCallback) {
            });
        ...
        YonYouJSBridge.registerHandler('backbutton', function (data, responseCallback) {
            callbackFun(data, responseCallback)
        });
        var data = {}
        YonYouJSBridge.send(JSON.stringify(data), function (responseData) {
...
//拦截返回按钮，不会触发原生返回事件
const blockbackbutton = (paramobj, callbackFun) => {
    YYEsnBridge.ready(function () {
        YYEsnBridge.registerHandler({
            'onCallBackButton': function (res) {
                callbackFun(res);
            }
        });
        if (!window.backInterceptRegistered) {
            YYEsnBridge.do('backIntercept', {
                'callback': 'onCallBackButton',
                'success': function (res) {
                    console.info('调用成功...' + res);
                },
                'fail': function (err) {
            ...
            window.backInterceptRegistered = true;
        }
    });
};
...
const setYYStorage = (key, data, callbackFun) => {
    YYEsnBridge.ready(function () {
        YYEsnBridge.do('setStorage', {
            'data': data,
            'key': key,
            'success': function (res) {
                callbackFun(res);
            ...
            'fail': function (err) {
                console.log('setStorage调用失败' + err);
            },
            'complete': function (res) {
                console.log('setStorage调用完成');
            }
        });
    });
};
...
//根据传入 key 值，获取缓存数据
const getYYStorage = (key, callbackFun) => {
    YYEsnBridge.ready(function () {
        YYEsnBridge.do('getStorage', {
            'key': key,
            'success': function (res) {
                callbackFun(res);
            ...
            'fail': function (err) {
                console.log('getStorage调用失败' + err);
            },
            'complete': function (res) {
                console.log('getStorage调用完成');
            }
        });
    });
};
...
//根据传入 key 值，删除缓存数据。
const removeYYStorage = (key, callbackFun) => {
    YYEsnBridge.ready(function () {
        YYEsnBridge.do('removeStorage', {
            'key': key,
            'success': function (res) {
                callbackFun(res);
            ...
            'fail': function (err) {
                console.log('removeStorage调用失败' + err);
            },
            'complete': function (res) {
                console.log('removeStorage调用完成');
            }
        });
    });
};
