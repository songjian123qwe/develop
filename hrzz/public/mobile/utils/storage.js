...
function getSaveData(str,key){
    if(!isNaN(key)){alert("第二个参数不合法，该类型应为sting类型")}
...
/**
 * 类路由方法，解决location.go(-1),在safari浏览器react的钩子函数不执行的问题
 * 指定"__hrRouterHistory"
 * 注：仅适用在/hrzz/目录下的页面跳转
 */
const hrRouter = {
    clear(){
        clearSession('__hrRouterHistory')
    },
    // 页面跳转，没有使用路由，直接url跳转
    toUrl(url){
    ...
    setRoot(url){
        ...
        href='/hrzz/'+href.split('/hrzz/')[1]
        let hrRouterHistory= getSession('__hrRouterHistory') || []
        hrRouterHistory=[href];
        setSession('__hrRouterHistory',hrRouterHistory)
    },
    // 跳转
    push(url){
        let hrRouterHistory= getSession('__hrRouterHistory') || []
        hrRouterHistory.push(url);
        setSession('__hrRouterHistory',hrRouterHistory)
        this.toUrl(url)
    },
    // 回退
    go(num){
        ...
        let hrRouterHistory= getSession('__hrRouterHistory') || []
        hrRouterHistory=hrRouterHistory.slice(0,index)
        setSession('__hrRouterHistory',hrRouterHistory)
        hrRouterHistory.reverse()
        if(hrRouterHistory.length<1){
            return
        }
        this.toUrl(hrRouterHistory[0])
    }
}
