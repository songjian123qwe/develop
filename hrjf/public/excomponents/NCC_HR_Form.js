...
class NCC_HR_Form extends Component{
    ...
    onChange(obj,value){
        ...
        if(!item){
            throw new Error(this.state.json['jf6005-000030'])/* 国际化处理： 没找到对应的元数据项*/
        }
        item = item[0];
        /*********************************************
    ...
    simpleValidate(item,itemData){
        ...
        if(!simpleRegExp.test(itemData.value)){
            this.state.validateFormData[item.attrcode] = {
                error:true,
                errorMessage:this.state.json['jf6005-000031']/* 国际化处理： 请输入正确格式的数据!*/
            };
        }else if(item.maxlength && itemData.value.replace(/[\u0391-\uFFE5]/g,"oo").length > item.maxlength){
            this.state.validateFormData[item.attrcode] = {
                error:true,
                errorMessage:this.state.json['jf6005-000032']/* 国际化处理： 输入值的长度超长!*/
            };
    ...
    getFormMetaItemByAttrCode(obj){
        ...
        let areacode = obj.areacode;
        if(!attrcode){
            throw new Error(this.state.json['jf6005-000033']);/* 国际化处理： 缺少attrcode参数！*/
        }
        if(!areacode){
            throw new Error(this.state.json['jf6005-000034']);/* 国际化处理： 缺少areacode参数！*/
        }
    ...
    setFormItemValue(areacode,item,data){
        if(!this.state.formData.hasOwnProperty(areacode)){
            throw new Error(this.state.json['jf6005-000035']+areacode+this.state.json['jf6005-000036']);/* 国际化处理： 未找到,对应的表单数据对象*/
        }
        if(!this.state.formData[areacode].hasOwnProperty(item.attrcode)){
            throw new Error(this.state.json['jf6005-000037']);/* 国际化处理： 该表单数据对象没有该数据项*/
        }
        if(!data){
            throw new Error(this.state.json['jf6005-000038']);/* 国际化处理： 请设置数据项数据*/
        }
                    ...
                    if(isEditStatus){
                        ...
                        value = (
                            <NCDatePicker
                                ...
                                placeholder={item.placeholder || this.state.json['jf6005-000039']}/* 国际化处理： 请选择合适的日期*/
                                onChange={this.onChange.bind(this,eventParam)}
                                value={value}
                            />)
                    }
