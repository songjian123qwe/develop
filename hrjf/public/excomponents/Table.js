...
var CheckBoxSelectModel = function(table,config){
    ...
    handerChecked = function(operRecords, operaiton = 'checked', suppressEvent = true){
        var records = recordids ? []: (mode === 'edit' ? table.getDirtyOperation().findRecordByIds(recordid) : table.findRecordByIds(recordid)),
...
class Table extends Component {
                    ...
                    var dataIndex = col.dataIndex || function(){},
                        render = (text, record, index) => {
                            var value = BaseUtils.isFunction(dataIndex) ? dataIndex(record, index) : record.getData()[dataIndex],
                                ...
                                return editer(record, index);  
                        };
                    return { ...col ,...{dataIndex: dataIndex,render:render}};
                });
    ...
    getDirtyOperation(){
        ...
        return {
            ...
            findRecordByIds(id){
                return this.state.dirtyRecords.filter(r => {
                    return id.indexOf(r.getId()) !== -1;
                });
            },
            ...
            addData: ( datas ) =>{
                ...
                state.dirtyRecords.concat( records.map(e =>new Record(data, 'add') ) );
                this.setState(state, () => {
